...
enum AccentColor: String, CaseIterable, Sendable {
    /// Follow the active theme's own accent (default). Ported themes read with their native hue.
    case theme
    case golden
    case blue
    case purple
    ...
    var lightHex: UInt {
        switch self {
        case .theme: ColorThemeStorage.current.palette.accent.lightHex
    ...
    var darkHex: UInt {
        switch self {
        case .theme: ColorThemeStorage.current.palette.accent.darkHex
